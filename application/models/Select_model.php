...
class Select_model extends CI_Model
{
    ...
    public function get_product_by_id($set_id)
    {
      ...
      $sql = "SELECT pm.id as id, pm.product_name as name, pm.product_category as category,pm.meal_type as meal,pm.product_sku as sku, pm.product_gst as progst,pm.product_sub_total as subttl, pm.product_price as price, pm.product_quantity as quantity, pm.is_customizable as custom,pm.meal_plan, cm.customizable_product as custom_product, cm.custom_product_price as custom_product_price,pi.image1,pi.image2,pi.image3,pi.image4,pi.image5 FROM product_master as pm LEFT JOIN custom_product_master as cm ON cm.product_sku = pm.product_sku LEFT JOIN product_image as pi ON pi.product_sku = pm.product_sku where pm.id=".$set_id.";";
      $query = $this->db->query($sql);
      return $query;
    }
    ...
    * --------------------------
    * REFACTOR 1 : 10062019
    *  * ADDED product_gst, product_total, custom_gst, custom_sub_price IN SQL JOIN
    */
   ...
   public function select_all_products()
   {
     ...
     pm.product_gst as product_gst,
     pm.product_sub_total as product_subtotal,
     ...
     cm.custom_product_gst AS custom_gst,
     cm.custom_product_subtotal AS custom_sub_price,
 ...
 public function get_product_list_on_user_id($get_user_id,$get_user_type,$set_confirmation)
 {
       ...
       $sql = "SELECT pc.order_id AS order_id, pc.unique_order_id AS unique_order_id, pc.rowid AS rowid, pc.product_sku AS sku, pc.product_name AS name,pc.product_gst AS gst,pc.product_subtotal AS subtotal,pc.price AS price,pc.quantity AS quantity,cc.main_product_price AS main_product_price, cc.optional_product_price AS optional_product_price,cc.product_gst AS main_product_gst FROM cart_confirmation AS cc LEFT JOIN ".$set_table." AS ipm ON ipm.id = '".$get_user_id."' LEFT JOIN product_cart AS pc ON pc.unique_order_id = cc.unique_order_id WHERE cc.confirm_flag = ".$set_confirmation." AND pc.user_id='".$get_user_id."';";
  ...
  public function get_optional_list_on_user_id($get_user_id,$get_user_type,$set_confirmation)
  {
   ...
   else
   {
     ...
     $sql = "SELECT opc.rowid AS optional_rowid,opc.product_sku AS optional_sku,opc.product_name AS optional_name,opc.product_gst AS optional_gst,opc.product_subtotal AS optional_subtotal,opc.price AS optional_price,opc.quantity AS optional_quantity FROM cart_confirmation AS cc LEFT JOIN ".$set_table." AS ipm ON ipm.id = '".$get_user_id."' LEFT JOIN optional_product_cart AS opc ON opc.unique_order_id = cc.unique_order_id WHERE cc.confirm_flag = '".$set_confirmation."' AND opc.user_id='".$get_user_id."';";
...
//25-2-19(Mounika)
  ...
  //$query = $this->db->query("SELECT cp.company_name as company,re.branch_name as branch,re.* FROM corporate_representative as re  left join company_data as cp on re.company_name = cp.id where cp.company_name = '$company'");
   ...
   public function select_grid_combo_id($get_store)
   {
     ...
     * -----------------------
     *  REFACTOR 1 : 10062019
     * * ADDED CURRENT DATE TO THE WHERE CONDITION
     * -----------------------
     */
    public function get_attendance_data_id($kitchen_id)
    {
       $current_date = date("Y-m-d");
       $sql = "SELECT * FROM kitchen_emp_attendance WHERE attendance_flag = '0' AND kitchen_id = '$kitchen_id' AND set_date = '$current_date'";
       $query = $this->db->query($sql);
       return $query;
    }
...
//12-04-19(Mounika)
   ...
   {
    $query = $this->db->query("SELECT id,emp_name FROM delivery_employee where approve = 1 and status =0");
    return $query;
   }
    ...
    public function select_kitchen_assigned_data($products,$date)
     {
       ...
       {
        $sql = "SELECT `id`, `kitchen_id`, `product_sku`, sum(`assigned_orders`) as assigned_orders  , `date` FROM kitchen_assigned_products WHERE product_sku = '".$products[$i]."' AND date ='".$date."' GROUP BY kitchen_id,date";
         $query = $this->db->query($sql);
         $j = 0;
         foreach($query->result() as $row)
         {
     ...
     // }
public function get_orders_order_id($userId,$setOrderId)
    {
     
      $data_array = array();

      $sql1 = " DROP VIEW IF EXISTS confirm_orders_view_".$userId;
      $sql2 = "CALL select_order_products(".$userId .",'".$setOrderId."');";
  ...
  //10-06-19 divya
    public function kitchen_total_orders_id($kid)
    {
      
      $sql = "SELECT product_sku AS sku, sum(assigned_orders) AS sum, date AS assigned_date FROM kitchen_assigned_products WHERE kitchen_id = '".$kid."' AND  order_flag = 0 AND date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) GROUP BY product_sku,date ";
      $query = $this->db->query($sql);
       return $query;
    }
    //13-06-19
    public function json_tester()
    {
      $sql = "select * from date_products";
      $query = $this->db->query($sql);
      return $query;
    }
