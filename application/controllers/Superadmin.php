      ...
      if($this->session->has_userdata('admin_id'))
        {
            ...
            $data['data_array'] = json_decode($json,TRUE);
            $data['get_array'] = $this->select_model->select_product_master(1);
            $this->load->view('headers/superadmin_home_header',$data);
            $this->load->view('superadmin/superadmin_products_count',$data);
            $this->load->view('headers/footer');
        }
        else
        {
      ...
      if($expected_count != $actual_count)
       ...
       if ($this->form_validation->run())
       {
          ...
          $set_product = array(
              ...
              'product_sub_total' => $this->input->post('product_total'),
          ...
          //products seperated by ,
                              ...
                              //gst seperated by ,
                              $get_customizable_gst_array = $this->input->post('customizable_gst[]');
                              $get_customizable_gst_string = implode(",",$get_customizable_gst_array);

                              //subtotal seperated by ,
                              $get_customizable_subtotal_array = $this->input->post('customizable_subtotal[]');
                              $get_customizable_subttl_string = implode(",",$get_customizable_subtotal_array);
                            ...
                            $set_is_customizable = array(
                              ...
                              'custom_product_gst' => $get_customizable_gst_string,
                              'custom_product_subtotal' => $get_customizable_subttl_string,
...
public function kitchen_inventory($error = null)
{
    ...
    $data['get_kitchen_id'] = $this->select_model->select_kitchen_register();
    $data['get_kitchen_products'] = $this->select_model->select_primary_products(null);
    $data['get_units_master'] = $this->select_model->get_units_master();
    ...
    */
    ...
    public function insert_assign_kitchen_products()
    {
      ...
      for($i = 0; $i < count($this->input->post('product_name[]'));$i++)
      {
        ...
        if($this->input->post('sub_product_name[]')[$i] == "None")
        {
          // echo $i;
          // echo "Inside If<br>";
          $super_array[$i] = array(
        ...
        elseif($this->input->post('sub_product_name[]')[$i] != "None")
        {
          
          // echo " Inside else If"."<br>";
         ...
         if(!isset($flag))
         {
          ...
          for($j = $i, $x = $j, $z = $x - 1; $j < count($this->input->post('sub_product_name[]')); $j++, $z++ ,$x++)
          {

            // echo "Else If Outer For";
            // echo " \$j=".$j."<br>";
            ...
            "product_name" => $this->input->post('product_name[]')[$x],
             "product_sku" => $this->input->post('product_sku[]')[$x],
             "bundled_flag" => 'bundle',
            "estimated_units" => 0
          );  
              ...
              $explode_product_name = explode(",",$this->input->post('sub_product_name[]')[$z]);
              $explode_product_sku = explode(",",$this->input->post('sub_product_sku[]')[$z]);

              for( $k = 0,$l = $j+1; $k < count($explode_product_name); $k++,$l++)
              {
                // echo "Inner For";
                // echo "\$k=".$k."<br>";
                $super_array[$l] = array(
                  "product_name" => $explode_product_name[$k],
                  "product_sku" => $explode_product_sku[$k],
                  "bundled_flag" => 'bundle',
                  "estimated_units" => 0
                );
              }
      ...
      for($a = 0; $a < count($super_array);$a++)
         {
          $validator = $this->validate_model->assigned_kitchen_product_validator($kitchen_id, $super_array[$a]['product_sku']);
          if($validator)
          {
            echo "<script> alert('Warning : ".$super_array[$i]['product_sku']." Already Exists'); </script>";
          }
          else
          {
          
            $insert_assigned_product_array = $this->insert_model->insert_assigned_kitchen_products($kitchen_id,$super_array[$a]);
...
/*----------------------Assign bulk Orders ----------------------------*/
public function assign_orders_kitchen()
{
  $data['user_data'] = $this->select_model->select_user($this->backend_table,$this->admin_id);
    $data['kitchen_ids'] = $this->select_model->select_kitchen_register();
      $this->load->view('headers/superadmin_home_header',$data);
      
      $this->load->view('superadmin/superadmin_assign_orders',$data);
      $this->load->view('headers/footer');
}
//fetching product sku
  function fetch_product_sku()
  { 
    
      $result_data = $this->select_model->fetch_product_sku($this->input->post('k_id'));
      echo json_encode($result_data->result());
    
  }

// //!DEPRECATED
// //fetching required product quantity
//   function fetch_required_qnt()
//   {
//     $result = $this->select_model->fetch_product_quantity($this->input->post('sku_val'),$this->input->post('date_val'));
//     echo $result;
//   }
    ...
    // public function insert_assigned_products()
    // {
    //   $insert_notify = array();
    //   $insert_products = array();
    //   $insert_products['kitchen_id'] = $this->input->post('kitchen_id');
    //   $insert_products['product_sku'] = $this->input->post('produt_sku');
    //   $insert_products['assigned_orders'] = $this->input->post('assign_orders');
    //   $insert_products['date'] = $this->input->post('date');

    //     $updater = array();
    //     $kitchen_id = $insert_products['kitchen_id'];
    //     $updater['product_sku'] = $insert_products['product_sku'];
    //     $updater['quantity'] = (integer)$insert_products['assigned_orders'];
    //     $updater['date'] = $insert_products['date'];
    //     $update_assigned_table = $this->update_model->update_date_products($updater,TRUE);
    //     if($update_assigned_table)
    //     {
    //       $return_value = $this->insert_model->insert_kitchen_assigned_products($insert_products);
    //       if($return_value)
    //         {

    //           $insert_notify['kitchen_id'] = $this->input->post('kitchen_id');
    //           $insert_notify['message'] = "we have assigned SKU Id ".$updater['product_sku']." Quantity of ".$updater['quantity']." For Date ".$updater['date'];
    //          $insert_notify['notify_flag'] = '0';
    //         $insert_note = $this->insert_model->insert_kitchen_notification($insert_notify);
    //           echo "<script> alert('Assigned ".$updater['quantity']." Products to Kitchen : ".$kitchen_id."'); </script>";
    //           redirect(base_url()."superadmin/orders_list",'refresh');
    //          }
    //     }
    // }
     ...
     * REFACTOR 1 : 03062019
     * -----------------------
     *  * ADDED TO DATE AND FROM DATE PARAMETERS TO fetch_product_quantity *
     */
    ...
    function fetch_required_qnt()
    {
        ...
        $sku = $this->input->post('sku_val');
        $result = $this->select_model->fetch_product_quantity($sku,$from_date,$to_date);
     ...
     * Refactor 1 : 03062019
     * -------------------------
     *  * MULTIPLE QUANTITIES ARE ASSIGNED ON MULTIPLE DATES *
     *  * ADDED A FOR LOOP FOR ARRAY OF DATES AND ASSIGNED QUANTITY *
     * ------------------------- 
     */
    ...
    public function insert_assigned_products()
    {
      ...
      for($i = 0; $i < count($this->input->post('date[]')); $i++)
      {
        $insert_products['kitchen_id'] = $this->input->post('kitchen_id');
        $insert_products['product_sku'] = $this->input->post('produt_sku');
        $insert_products['assigned_orders'] = $this->input->post('assign_orders[]')[$i];
        $insert_products['date'] = $this->input->post('date[]')[$i];
    ...
    public function manage_assign_orders()
    {
       $data['user_data'] = $this->select_model->select_user($this->backend_table,$this->admin_id);
       $this->load->view('headers/superadmin_home_header',$data); 
        $data['kitchen_data'] = $this->select_model->select_kitchen_register();
       $this->load->view('superadmin/manage_assign_orders',$data);
       $this->load->view('headers/footer');   
    }
    //fetch manage assign 
    public function fetch_assign_orders()
    {
      $result_data = $this->select_model->fetch_assign_product($this->input->post('kit_id'));
    ...
    //undo assignment products
    public function undo_assign()
    {
      $get_undo_data =array();
      $get_undo_data['id'] = $this->input->post('id');
      $get_undo_data['product_sku'] = $this->input->post('sku_id');
      $get_undo_data['quantity'] = $this->input->post('orders_val');
      $get_undo_data['date'] = $this->input->post('date_val');

      $undo_data = $this->delete_model->delete_assigned_data($get_undo_data['id']);
      if($undo_data)
      {
        $get_undo_update = $this->update_model->update_date_products($get_undo_data,FALSE);
        if($get_undo_update)
        {
          echo $get_undo_update;
        }
      }
...
/* --------------------------kitchen notification ----------------------------*/
  public function kitchen_notification()
  {
     if($this->session->has_userdata('admin_id'))
          {
             $data['user_data'] = $this->select_model->select_user($this->backend_table,$this->admin_id);
               ...
               $this->load->view('headers/superadmin_home_header',$data);
               $this->load->view('superadmin/notification_kitchen',$data);
               $this->load->view('headers/footer');
          }
          else
          {
  ...
  public function notify_kitchen_data()
  {
        $data = array(
        ...
        print_r($data);
        $insert = $this->insert_model->insert_kitchen_notification($data);
        if($insert)
          {
            echo "<script> alert('Inserted Successfully'); </script>";
            //echo "<script>window.location.href='".base_url()."superadmin/manage_notification';</script>";
   ...
   /**
     ...
     public function ajaxCallGetassigned()
     {
       $date = $this->input->post('date');
       $products = $this->input->post('products');
       array_pop($products);
       $get_data = $this->select_model->select_kitchen_assigned_data($products,$date);
       echo($get_data);
     }
