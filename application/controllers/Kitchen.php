...
class Kitchen extends CI_Controller 
{
  ...
  public function notify_count()
  {
    ...
    {
    $data['notify_cnt'] = $this->select_model->fetch_kitchen_notification($this->session->userdata('kitchen_id'));
	...
	public function kitchen_login()
	{
     ...
     //   $data['note_data'] = $this->select_model->fetch_kitchen_notification_message($kit_id);
					// $this->load->view('headers/kitchen_home_header',$data);
					// $this->load->view('kitchen/kitchen_home',$data);
					// $this->load->view('headers/footer');
          redirect(base_url()."kitchen/kitchen_home");
				}
				else
				{
					$this->error_403();
  ...
  //24-5-2019(divya) changes
    ...
    if($this->session->has_userdata('kitchen_id'))
     ...
     // $this->session->set_userdata($notify_cnt);
                $kit_id = $this->session->userdata('kitchen_id');
       $data['note_data'] = $this->select_model->fetch_kitchen_notification_message($kit_id);
       $data['total_order'] = $this->select_model->kitchen_total_orders_id($this->session->userdata('kitchen_id'));
        $this->load->view('headers/kitchen_home_header',$data);
        $this->load->view('kitchen/kitchen_home',$data);
        $this->load->view('headers/footer');
      }
  ...
  //24-05-2019
    ...
    {
        $get_affected_rows = $this->update_model->update_notify($this->uri->segment(3));
        if($get_affected_rows)
        {
           echo "<script> alert('Deleted Successfully'); </script>";
              echo "<script>window.location.href='".base_url()."kitchen/kitchen_home/';</script>";
        }
    }
    //19-2-2019(Mounika)
    ...
    public function add_stock()
    {
        ...
        $reduce_qnt = array(
           "product_sku"=>$this->input->post('sku'),
          "quantity" => $this->input->post("currquant")
        );
        ...
        if($get_count > 0)
        {
            ...
            if($stock)
            {
                ...
                if($get_affected)
                {
                 ...
                 $get_quant = $this->update_model->update_stock_add_primary($reduce_qnt);
                if($get_quant)
                {
                    echo "success";
                }
    ...
    public function kitchen_attendance()
    {
      ...
      $insert_attendance_array = array();
            ...
            {
              if(empty($insert_attendance_array))
              {
                echo "<script> alert('NO Employees'); </script>";
               }
            }
            // echo "<pre>";
            ...
            $data['select_current_attendance'] = $this->select_model->get_attendance_data_id($this->session->userdata('kitchen_id'));
      ...
      if($this->session->userdata('kitchen_id'))
        {
          $data['notify_cnt'] = $this->select_model->fetch_kitchen_notification($this->session->userdata('kitchen_id'));
           $this->load->view('headers/kitchen_home_header',$data);
           $kitchen_id = $this->session->userdata('kitchen_id');
           $data['assigned_data'] = $this->select_model->select_assigned_count($kitchen_id);
           $this->load->view('kitchen/assigned_orders',$data);
           $this->load->view('headers/footer');
          }
        else
        {
    ...
    */
    ...
    //======================================//
    /* Ajax Call to update orders as cooked */
    //======================================//

    public function update_assigned_product_status()
    {
        $updater = array();
        $updater["kitchen_id"] = $this->input->post('kitchen_id');
        $updater["product_sku"] = $this->input->post('sku');
        $updater['date'] = $this->input->post('date');

        $get_updater = $this->update_model->update_assigned_product_status($updater);
        if($get_updater)
        {
          redirect(base_url()."kitchen/assigned_orders",'refresh');
        }
    }

    //24-05-19
    public function view_kitchen_data()
   {
    if($this->session->userdata('kitchen_id'))
          {
              ...
              $data['note_data'] = $this->select_model->fetch_kitchen_notification_message($kit_id);
               $this->load->view('headers/kitchen_home_header',$data);
               $this->load->view('kitchen/kitchen_notify',$data);
               $this->load->view('headers/footer');
          }
          else
          {
  ...
  //24-05-19
  public function create_kitchen_notification()
  {
     if($this->session->has_userdata('kitchen_id'))
          {
            $kit = $this->session->userdata('kitchen_id');
             $data['notify_cnt'] = $this->select_model->fetch_kitchen_notification($this->session->userdata('kitchen_id'));
            $data['notify_data'] = $this->select_model->receive_kitchen_notification_message($kit);
               $this->load->view('headers/kitchen_home_header',$data);
               $this->load->view('kitchen/create_kitchen_notify',$data);
               $this->load->view('headers/footer');
          }
          else
          {
  ...
  public function notify_kitchen_data()
  {
        ...
        if($insert)
          {
            echo "<script> alert('Inserted Successfully'); </script>";
            echo "<script>window.location.href='".base_url()."kitchen/create_kitchen_notification';</script>";
          }
  ...
  //   if($this->session->has_userdata('kitchen_id'))
  //     {
  //       $kit = $this->session->has_userdata('kitchen_id')
  //       //$data['notify_data'] = $this->select_model->fetch_kitchen_notification_message($kit);
  //       $data['notify_cnt'] = $this->select_model->fetch_kitchen_notification($kit);
